...
# NOTE: Disabled until custom metrics exist (apps must emit custom.googleapis.com/api/request/count)
...
# High Latency Alert
# NOTE: Disabled until custom metrics exist (apps must emit custom.googleapis.com/api/request/latency)
...
# NOTE: Disabled until custom metrics exist (apps must emit custom.googleapis.com/worker/queue/depth)
...
#     condition_threshold {
#       filter          = "resource.type=\"global\" AND metric.type=\"custom.googleapis.com/worker/queue/depth\""
#       comparison      = "COMPARISON_GT"
...
# NOTE: Disabled until custom metrics exist (apps must emit custom.googleapis.com/health/status)
...
#     condition_threshold {
#       filter          = "resource.type=\"global\" AND metric.type=\"custom.googleapis.com/health/status\" AND metric.labels.service=\"hta-${each.key}\" AND metric.labels.status=\"unhealthy\""
#       comparison      = "COMPARISON_GT"
#       threshold_value = 0
#       duration        = "120s"
#
