...
import { evaluateExpression } from '@/lib/certificate/fields'
import {
  type Accuracy,
...
export interface AccuracyContext {
  ...
  /** max - min, where an accuracy is stated against the span rather than full scale. */
  span?: number | null
}
 ...
 * list: these four are what an admin may type, and nothing else resolves.
 */
function expressionValues(ctx: AccuracyContext): Record<string, string> {
  const at = (v: number | null | undefined) =>
    v === null || v === undefined || !Number.isFinite(v) ? '' : String(Math.abs(v))
  return {
    reading: at(ctx.reading),
    'full scale': at(ctx.fullScale),
    span: at(ctx.span),
    'least count': at(ctx.leastCount),
  }
}
...
export function resolveAccuracy(
  ...
  /**
   * Two bounds, and the comparison has to answer with one number.
   *
   * It answers with the worse of the two. Taking the smaller would claim the master is
   * better than it is wherever the error lands on the other side, and the point of the
   * ratio is to be safe rather than flattering.
   */
  if (accuracy.type === 'asymmetric') {
    const bounds = [accuracy.upper, accuracy.lower]
      .filter((b): b is number => b !== null && Number.isFinite(b))
      .map(Math.abs)
    return bounds.length ? { value: Math.max(...bounds), unit: accuracy.unit } : null
  }
  ...
  /**
   * An expression states the accuracy exactly, so where there is one it wins.
   *
   * The same parser the certificate's result columns use - a small recursive-descent
   * one, never eval - so an expression an admin types is evaluated the same way
   * everywhere and reaches no JavaScript interpreter. It returns null on anything it
   * cannot work out, including a name with no value behind it, which is the same
   * "cannot be reduced to a number" answer the fields below give.
   */
  if (accuracy.evaluable && accuracy.evaluable.trim()) {
    const value = evaluateExpression(accuracy.evaluable, expressionValues(ctx))
    return value === null ? null : { value: Math.abs(value), unit: null }
  }
  ...
  if (accuracy.percent_value != null) {
    ...
    } else if (accuracy.percent_of === 'full_scale') {
      if (ctx.fullScale == null) return null
      total += accuracy.percent_value * Math.abs(ctx.fullScale)
    } else if (accuracy.percent_of === 'span') {
      // The span where one is given, and the full scale otherwise - which is the same
      // number whenever the range starts at zero, and most do.
      const basis = ctx.span ?? ctx.fullScale
      if (basis == null) return null
      total += accuracy.percent_value * Math.abs(basis)
    } else {
...
export function evaluateSuitability(
  ...
  const ranges: RangeSuitability[] = required.map((range) => {
    ...
    // Same three pieces of context the badge needs, from the same place: the declared
    // span for a "% of full scale" accuracy, the band's own least count for a digits
    // term. Omitting either turns a perfectly stated accuracy into "cannot be reduced
    // to a number", which is the one message an engineer cannot act on.
