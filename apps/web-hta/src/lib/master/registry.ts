...
// scripts/registry-build/standardize_registry.py from the working registry under reference_docs/.
//
...
export interface FormulaAccuracy {
  type: 'formula'
  /**
   * What the calibrating lab wrote. A certificate reproduces this exactly, so it is
   * never re-worded, and it is kept even where no number could be got out of it.
   */
  expression: string
  /**
   * The same accuracy as arithmetic the app can evaluate, over {reading},
   * {full scale}, {span} and {least count}.
   *
   * Present for the shapes the three fields below cannot hold - a constant term
   * ("+/-(330ppm + 1% rdg)"), or two percentages on two bases ("+/-(3% of reading +
   * 0.3% FS)"). Where it is present it wins, because it is the more exact statement;
   * where it is absent the fields below are used as before.
   */
...
/**
 * Accuracy that is not the same either side of the reading, e.g. +0.5 / -0.2.
 *
 * Collapsing the two to the larger would overstate the instrument on one side and
 * understate it on the other, and nothing downstream could tell which it had been
 * handed - so they are kept apart, and a comparison takes the worse of the two.
 */
export interface AsymmetricAccuracy {
  type: 'asymmetric'
  upper: number | null
  lower: number | null
  unit: string
...
export interface RegistryUnit {
   ...
   * number and position by scripts/registry-build/link_legacy_master_ids.py, which fails rather than
   * emit a null.
   */
  legacy_id: number
...
export function isAsymmetric(a: Accuracy | null): a is AsymmetricAccuracy {
  return a?.type === 'asymmetric'
}
...
export function isComparable(a: Accuracy | null): boolean {
  ...
  if (a.type === 'asymmetric') return a.upper !== null || a.lower !== null
  if (a.type === 'formula') {
    if (a.evaluable && a.evaluable.trim()) return true
    return a.percent_of !== null || a.digits !== null
  }
  return false
}
