...
export function ReviewerCertificateTable() {
  const [certificates, setCertificates] = useState<ReviewCertificateItem[]>([])
  ...
  const fetchCertificates = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/certificates/reviewer?${params}`)
      if (res.ok) {
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-2.5 items-stretch sm:items-center mb-[18px]">
        ...
        <Select value={statusFilter} onValueChange={handleStatusFilter}>
          <SelectTrigger className="h-10 w-full sm:w-[200px] rounded-[9px] border-border bg-white text-sm text-[#64748b]" aria-label="Filter by status">
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-[#f8fafc] border-b-2 border-[#f1f5f9]">
                  ...
                  <th className="text-right px-4 py-[11px] text-[11px] font-bold uppercase tracking-[0.06em] text-[#94a3b8]">
                    Actions
                  </th>
                </tr>
              </thead>
