...
async function createOfflineEditDraftFromCache(
  ...
  await db.run(
    `INSERT INTO drafts (
      ...
      calibrated_at,
      date_of_calibration, calibration_start_time, calibration_end_time,
      calibration_due_date, calibration_tenure,
    ...
    data.dateOfCalibration || null, data.calibrationStartTime || null, data.calibrationEndTime || null,
    data.calibrationDueDate || null, data.calibrationTenure ?? 12,
...
export function registerDraftHandlers(): void {
  ...
  ipcMain.handle('draft:create', async (_event, data: CreateDraftInput | undefined) => {
    ...
    await db.run(
      ...
      draftData.dateOfCalibration || null, draftData.calibrationStartTime || null, draftData.calibrationEndTime || null,
      draftData.calibrationDueDate || null, draftData.calibrationTenure ?? 12,
...
export function registerConflictHandlers(): void {
  ...
  ipcMain.handle('draft:resolve-conflict', async (_event, draftId: string, resolvedData: SaveDraftInput & { parameters?: ParameterInput[] }) => {
    ...
    await db.run(
      ...
      resolvedData.dateOfCalibration || null, resolvedData.calibrationStartTime || null, resolvedData.calibrationEndTime || null,
      resolvedData.calibrationDueDate || null, resolvedData.calibrationTenure ?? 12,
